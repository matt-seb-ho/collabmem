...
if version.parse(trl.__version__) > version.parse("0.10.1"):
    raise RuntimeError(
        f"For this script, `trl` version {trl.__version__} is incompatible. Please install trl<-0.10.1 to run this script."
    )
...
def main() -> None:
    ...
    def compute_rewards(prompts, responses):
        ...
        for prompt, response in zip(prompts, responses):
            ...
            chat_history = multiturn_data["prompt"] + [
            ...
            rewards.append(
                torch.tensor(
                    np.mean(reward_info["MR"]), device=trainer.accelerator.device
                )
            )
    ...
    def generate_vllm(trainer, model, prompts):
        ...
        generation_kwargs.update(
            {"n": 1, "top_k": 50, "top_p": 1.0, "detokenize": False}
        )
        ...
        completion_ids = [
            torch.LongTensor(output.outputs[0].token_ids) for output in outputs
        ]
    ...
    for batch in tqdm(dataloader(), total=total_steps):
        ...
        # Generate responses
        model.train()
        prompt_ids, completion_ids = generate_vllm(
            trainer, model.pretrained_model, prompts
        )
        ...
        trainer.log_stats(
            stats, batch, rewards, columns_to_log=["prompt", "responses", "rewards"]
        )
        ...
        print(
            f"[Rank: {local_rank}] Step: {step}, Mean Reward: {np.mean([r.item() for r in rewards]):.4f}"
        )
