    ...
    def generate_response(self, conversation, sample, temperature=1.0):
        ...
        if num_user_msgs == 0:
            ...
            shards = sample["shards"][
                1:
            ]  # filter out the first shard, which is the initial query
            shard_ids = [shard["shard_id"] for shard in shards]
            shard_ids_revealed = [
                shard_id for shard_id in shard_ids if shard_id in shard_revealed_ids
            ]
            shard_ids_not_revealed = [
                shard_id for shard_id in shard_ids if shard_id not in shard_revealed_ids
            ]

            shard_texts_revealed = [
                shard for shard in shards if shard["shard_id"] in shard_ids_revealed
            ]
            shard_texts_not_revealed = [
                shard for shard in shards if shard["shard_id"] in shard_ids_not_revealed
            ]
            shards_revealed_str = json.dumps(shard_texts_revealed)
            shards_not_revealed_str = json.dumps(shard_texts_not_revealed)

            user_agent_prompt_populated = (
                self.prompt_response.replace(
                    "[[CONVERSATION_SO_FAR]]",
                    extract_conversation(conversation, to_str=True, skip_system=True),
                )
                .replace("[[SHARDS_REVEALED]]", shards_revealed_str)
                .replace("[[SHARDS_NOT_REVEALED]]", shards_not_revealed_str)
            )
            response_obj = generate_json(
                [{"role": "user", "content": user_agent_prompt_populated}],
                model=self.model,
                timeout=100,
