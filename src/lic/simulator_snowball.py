        ...
        while not is_completed:
            revealed_shard_ids = set(
                [
                    ...
                    if msg["role"] == "log"
                    and msg["content"]["type"] == "hint_revealed"
                ]
            )
            ...
            if all_shards_revealed:
                if verbose:
                    print_colored(
                        f"[log] all hints revealed ({revealed_shard_ids} / {len(shards)})",
                        "blue",
                    )
            ...
            user_response, hint_revealed_id, cost_usd = (
                self.user_agent.generate_response(self.trace, self.sample)
            )
            ...
            if len(revealed_shard_ids) > 0:
                ...
                user_response = self.user_response_template.format(
                    HINTS_SO_FAR="\n".join(
                        [f"- {hint}" for hint in user_response_history[:-1]]
                    ),
                    LAST_HINT=user_response_history[-1],
                )
            ...
            if system_verification_response["response_type"] == "answer_attempt":
                ...
                if verbose:
                    print_colored(
                        f"[log] answer evaluation:\n```{extracted_answer}\n```\n({'correct' if is_correct else 'incorrect'}; score: {score})",
                        "blue",
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of snowball runs per model",
    )
    parser.add_argument(
        "--N_workers",
        type=int,
    ...
    for assistant_model in args.models:
        ...
        for sample in samples:
            ...
            ] * (args.N_snowball_runs - run_counts[sample["task_id"]])
