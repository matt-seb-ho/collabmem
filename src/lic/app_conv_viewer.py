...
def get_conversation_stats(conversations):
    ...
    avg_turns = (
        sum(
            len([t for t in conv["trace"] if t["role"] == "user"])
            for conv in conversations
        )
        / total_convs
    )
    ...
    for turn in conversation.get("trace", []):
        ...
        if role == "user":
            ...
            if log_type == "answer-evaluation":
                ...
                if "is_correct" in content and (
                    type(content["is_correct"]) == bool
                    or type(content["is_correct"]) == int
                ):
                ...
                st.chat_message("system").write(
                    f"Response classified as: {content['response']['response_type']}"
                )
...
def main():
    ...
    try:
        ...
        selected_task = st.sidebar.radio("Select Task", options=sorted(tasks))

        task = get_task(selected_task)
        dataset_fn = task.get_dataset_file().split("/")[-1]

        # Get available tasks for selected type
        available_conv_types = [
            d
            for d in os.listdir(f"{LOG_PATH}/{selected_task}")
        ...
        selected_type = st.sidebar.radio(
            "Select Conversation Type", options=sorted(available_conv_types)
        )
        ...
        selected_model = st.sidebar.selectbox(
            "Select Assistant Model", options=sorted(grouped_conversations.keys())
        )
        ...
        filter_incorrect = st.sidebar.checkbox(
            "Show only incorrect conversations", value=False
        )

        if filter_incorrect:
            all_model_conversations = [
                conv
                for conv in all_model_conversations
                if not conv.get("is_correct", True)
            ]
        ...
        selected_task_id = st.sidebar.selectbox(
            "Select Task ID", options=sorted(grouped_by_task_id.keys())
        )
        ...
        selected_conv_id = st.sidebar.selectbox(
            "Select Conversation ID", options=sorted(conversation_ids)
        )

        # Display statistics for selected model
        st.sidebar.header("Model Statistics")
        total_convs, solved_convs, success_rate, avg_turns = get_conversation_stats(
            all_model_conversations
        )
        ...
        selected_conversation = [
            conv for conv in model_conversations if conv["conv_id"] == selected_conv_id
        ][0]
