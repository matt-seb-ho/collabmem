...
def evaluate_insights(
    ...
    eval_prompt_fn="prompts/summary/summhay_evaluation.txt",
):
    ...
    bullets_str = json.dumps(
        {
            "bullets": [
                {"bullet_id": i + 1, "text": bullet} for i, bullet in enumerate(bullets)
            ]
        },
        indent=1,
    )
    ...
    for insight in insights:
        response_all = generate_json(
            [{"role": "user", "content": prompt_eval}],
            model=evaluator_model_card,
            return_metadata=True,
            variables={"BULLETS": bullets_str, "INSIGHT": insight["insight"]},
        )
...
def build_ref_insight2docids(topic):
    ...
    insight_id2references = {
        k: list(v) for k, v in insight_id2references.items()
    }  # make it into a list
...
def compute_single_sample_scores(
    summary, evals, insightid2ref_citations, partial_score=0.5, cite_offset=0
):  # the cite offset should be one for the annotators (but not for the model eval)
    bullets_obj = summary2bullets(summary)
    bullets = bullets_obj["bullets"]
    ...
    for e in evals:
        ...
        if e["coverage"] in ["PARTIAL_COVERAGE", "FULL_COVERAGE"]:
            ...
            gen_citations = set(
                [cite + cite_offset for cite in extract_citations(bullet_match)]
            )
            ...
            P = (
                ...
                else len(gen_citations & ref_citations) / len(gen_citations)
            )
            R = (
                0
                if len(ref_citations) == 0
                else len(gen_citations & ref_citations) / len(ref_citations)
            )
    ...
    return {
        ...
        "citation_precision": citation_precisions,
        "citation_recall": citation_recalls,
        "trim_ratio": trim_ratio,
    }


def compute_single_sample_results(
    summary, evals, insightid2ref_citations, partial_score=0.5, cite_offset=0
):
    scores = compute_single_sample_scores(
        summary, evals, insightid2ref_citations, partial_score, cite_offset=cite_offset
    )
