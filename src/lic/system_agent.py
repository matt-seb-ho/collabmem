...
class SystemAgent:
    def __init__(self, task_name, system_model, sample):
        ...
        assert self.answer_extraction_strategy in [
            "full_response",
            "prefix_suffix",
            "gen",
            "task_specific",
        ...
        with open(
            "prompts/system_answer_extraction_gen.txt", "r", encoding="utf-8"
        ) as f:
            self.answer_extraction_prompt_gen = f.read()
        with open(
            "prompts/system_answer_extraction_prefix_suffix.txt", "r", encoding="utf-8"
        ) as f:
    ...
    def verify_system_response(self, conversation_so_far):
        ...
        last_turn_text = extract_conversation(
            conversation_so_far, to_str=True, only_last_turn=True
        )

        # print("--------------------- TURN CLASSIFICATION ---------------------")
        # print(last_turn_text)

        system_verification_prompt_populated = (
            self.system_verification_prompt.replace(
                "[[CONVERSATION_SO_FAR]]", last_turn_text
            )
            .replace("[[INITIAL_SHARD]]", initial_query)
            .replace("[[SHARDS]]", json.dumps(shards))
            .replace("[[ANSWER_DESCRIPTION]]", self.answer_description)
        )
        system_verification_response_obj = generate_json(
            [{"role": "user", "content": system_verification_prompt_populated}],
        ...
        return system_verification_response, system_verification_response_obj[
            "total_usd"
        ]

    def extract_answer(self, conversation_so_far):
        assistant_response = [
            msg["content"] for msg in conversation_so_far if msg["role"] == "assistant"
        ][-1]
        ...
        if self.answer_extraction_strategy == "full_response":
            ...
            prompt = (
                self.answer_extraction_prompt_gen
                if self.answer_extraction_strategy == "gen"
                else self.answer_extraction_prompt_prefix_suffix
            )
            last_assistant_turn_text = extract_conversation(
                conversation_so_far, to_str=True, only_last_turn=True
            )
            ...
            while (
                ...
                and extraction_attempts < self.max_extraction_attempts
            ):
                extraction_attempts += 1
                answer_extraction_response_obj = generate_json(
                    [{"role": "user", "content": prompt}],
                    model=self.system_model,
                    return_metadata=True,
                    variables={
                        "ASSISTANT_RESPONSE": last_assistant_turn_text,
                ...
                if self.answer_extraction_strategy == "gen":
                    ...
                    if (
                        "[...]" in extractor_response
                        and extractor_response.count("[...]") == 1
                    ):
                        ...
                        extracted_answer = assistant_response[
                            start_idx : (end_idx + len(suffix))
                        ]
                    else:
                ...
                if (
                    ...
                    and extracted_answer not in assistant_response
                ):
