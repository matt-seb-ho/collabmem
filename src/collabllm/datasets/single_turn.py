...
class SingleTurnDataset:
    ...
    def __init__(
        self, data: List[Dict[str, Any]], eval_ratio: float = 0.1, seed: int = 42
    ):
        """
    ...
    def to_hf_dataset(self) -> DatasetDict:
        ...
        if "split" in self.fields:
            ...
            eval_indices = random.sample(
                range(len(self.data)), k=min(eval_size, len(self.data))
            )
            train_indices = list(set(range(len(self.data))) - set(eval_indices))

            split_indices = {"train": train_indices, "eval": eval_indices}
            ...
            dataset_dict[split] = Dataset.from_dict(
                {
                    ...
                    "single_turn_completion": [
                        self.data[i]["completion"] for i in indices
                    ],
    ...
    def get_splits_info(self) -> Dict[str, int]:
        ...
        if "split" in self.fields:
            ...
            eval_size = int(len(self.data) * self.eval_ratio)
            return {"train": len(self.data) - eval_size, "eval": eval_size}
